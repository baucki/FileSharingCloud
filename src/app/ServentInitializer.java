...
public class ServentInitializer implements Runnable {
	...
	public void run() {
		...
		if (someServentPort == -1) { //bootstrap gave us -1 -> we are first
			AppConfig.timestampedStandardPrint("First node in Custom system.");
			List<ServentInfo> nodes = new ArrayList<>();
			nodes.add(AppConfig.myServentInfo);
			AppConfig.customState.addNodes(nodes);
